...
K:${p.k} A:${p.a} M:${p.m} —
<span class="${colorClass}">KD ${kd.toFixed(2)}</span>
</div>`;
}).join("");
}
...
let s1Class = m.s1 > m.s2 ? "win" : "lose";
let s2Class = m.s2 > m.s1 ? "win" : "lose";

return `
<div>
${m.t1} <span class="${s1Class}">${m.s1}</span>
 -
<span class="${s2Class}">${m.s2}</span>
 ${m.t2}
</div>`;
}).join("");
}
